...
for pos_idx = 1:length(positions)
    ...
    [rhoAct, pRhoAct] = corr(Fig1_Data.PreLocomotionActivity, Fig1_Data.PostLocomotionActivity, 'Type', 'Spearman', 'Rows', 'complete');
    ...
    title({sprintf('Pre vs Post Locomotion Activity - %s', position), sprintf('ratio = %.3f,  p_{signrank} = %.3g,  rho = %.3f,  p_{rho} = %.3g', ratioAct, pAct, rhoAct, pRhoAct), ...
    ...
    pre_data = Fig1_Data.PreLocomotionActivity;
    post_data = Fig1_Data.PostLocomotionActivity;
    ...
    title(gca, {sprintf('Locomotion Activity %s: p = %.3g, g = %.3f (%s)', position, pAct, hedges_g, effect_interp), ...
    ...
    if ~isempty(Fig5_Data.TriggeredLocomotion) && ~isempty(Fig5_Data.TriggeredActivity)
        ...
        stdActivity = nanstd(Fig5_Data.TriggeredActivity, [], 1);
        
        plot(timeVector, meanActivity, 'LineWidth', 3, 'Color', params.colour(7,:));
        hold on;
        fill([timeVector, fliplr(timeVector)], [meanActivity + stdActivity, fliplr(meanActivity - stdActivity)], ...
...
function [Fig6_Data] = extractFrameByFrameActivityData(conditions, ActivityData, Behav, Skip, position, Performance, LocomotionFrameRate, threshold)
