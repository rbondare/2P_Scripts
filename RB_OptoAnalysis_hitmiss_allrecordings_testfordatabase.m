...
% Variable naming:   AnimalRB_MMDD_HHMM_opto or AnimalRB_MMDD_HHMM_baseline
...
% INCLUSION LIST - Only analyze these sessions
INCLUDED_SESSIONS = {
    'AnimalRB12_1103_1242_opto';
    'AnimalRB12_1104_1054_baseline';
    'AnimalRB12_1105_1150_opto';
    'AnimalRB12_1106_1157_opto';
    'AnimalRB12_1107_1023_baseline'; 
    'AnimalRB12_1111_1053_baseline' 
 %   'AnimalRB12_1112_1603_opto'; % 10 mW
 %   'AnimalRB12_1113_1423_opto'; % to use this trial need to use only up trial 100 (afterwards motor stopped working) 10 mW
 %   'AnimalRB12_1114_1231_opto'; % 5 mW
  %  'AnimalRB12_1117_1600_opto'; % here opto happens at different time (-0.5s) 10 mW
...
% Leave empty to include ALL sessions for this animal
% INCLUDED_SESSIONS = {};

% FIND AND FILTER SESSIONS
allVars = fieldnames(recordings);
animalVars = {};

if isempty(INCLUDED_SESSIONS)
    % No filter - include all sessions for this animal
    for i = 1:length(allVars)
        if contains(allVars{i}, ANIMAL_ID)
            animalVars{end+1} = allVars{i};
        end
    end
else
    % Only include sessions in the inclusion list
    for i = 1:length(INCLUDED_SESSIONS)
        sessionName = INCLUDED_SESSIONS{i};
        if isfield(recordings, sessionName)
            animalVars{end+1} = sessionName;
        else
            warning('Session not found in recordings:    %s', sessionName);
        end
    end
end

allVars = sort(animalVars);
...
if nSessions == 0
    error('No sessions found to analyze');
end
...
for s = 1:nSessions
    ...
    try
        ...
        % Calculate prestim licking
        [preStimLicking, ~] = calculatepreStimLicking(LicksInFrame, discardedTrials, ...
            FA_WINDOW_START, FA_WINDOW_END, nTrials);
        
        % Extract date label (MMDD from AnimalRB_MMDD_HHMM_*)
        ...
        if isBaselineSession
            ...
            % PreStimLicks:  include ALL trials (not filtered by discard)
            idx_all = true(1, nTrials);
            sessionData(s).FARate = sum(preStimLicking(idx_all)) / max(sum(idx_all), 1);
            ...
            % PreStimLicks: include ALL trials (separate by opto but no discard filter)
            idx_ctrl_all = (optoTrials == 0);
            idx_opto_all = (optoTrials == 1);
            sessionData(s).control.FARate = sum(preStimLicking(idx_ctrl_all)) / max(sum(idx_ctrl_all), 1);
            sessionData(s).opto.FARate = sum(preStimLicking(idx_opto_all)) / max(sum(idx_opto_all), 1);
...
%% PLOT 4: PRESTIM LICKING RATES
plotpreStimLicking(sessionData, ANIMAL_ID);
...
function [preStimLicking, preStimLicks] = calculatepreStimLicking(LicksInFrame, ...   
    discardedTrials, faWindowStart, faWindowEnd, nTrials)
    
    preStimLicking = false(1, nTrials);
    
    for t = 1:nTrials
        % DO NOT skip discarded trials - count ALL prestim licks
        ...
        if ~isempty(prematureLicks)
            preStimLicking(t) = true;
        end
    end
    
    % Calculate rate as proportion of ALL trials (including discarded)
    preStimLicks = sum(preStimLicking) / nTrials;
end
...
function plotOutcomesOverall(sessionData, animalID)
    % Extract opto sessions only - handle struct carefully
    optoIdx = [];
    for i = 1:length(sessionData)
        if isfield(sessionData(i), 'isBaseline') && ~isempty(sessionData(i).isBaseline) && ~sessionData(i).isBaseline
            if isfield(sessionData(i), 'control') && isfield(sessionData(i), 'opto')
                optoIdx = [optoIdx, i];
            end
        end
    end
    
    if isempty(optoIdx), return; end
    
    nSessions = length(optoIdx);
    ctrl_data = zeros(nSessions, 3);
    opto_data = zeros(nSessions, 3);
    
    for i = 1:nSessions
        idx = optoIdx(i);
        ctrl_data(i,: ) = [sessionData(idx).control.hitRate, ... 
                          sessionData(idx).control.missRate, ...
                          sessionData(idx).control.unattendedRate];
        opto_data(i,:) = [sessionData(idx).opto.hitRate, ...
                          sessionData(idx).opto.missRate, ...
                          sessionData(idx).opto.unattendedRate];
    end
...
function plotOutcomesAggregated(sessionData, animalID)
    ...
    for i = 1:nSessions
        ...
        ctrl_hit(i) = sessionData(idx).control.hitRate;
        ctrl_miss(i) = sessionData(idx).control.missRate;
        ctrl_unatt(i) = sessionData(idx).control.unattendedRate;
        
        opto_hit(i) = sessionData(idx).opto.hitRate;
        opto_miss(i) = sessionData(idx).opto.missRate;
        opto_unatt(i) = sessionData(idx).opto.unattendedRate;
        
        ctrl_hit_right(i) = sessionData(idx).control.hitRate_right;
        ctrl_miss_right(i) = sessionData(idx).control.missRate_right;
        ctrl_unatt_right(i) = sessionData(idx).control.unattendedRate_right;
        ctrl_hit_left(i) = sessionData(idx).control.hitRate_left;
        ctrl_miss_left(i) = sessionData(idx).control.missRate_left;
        ctrl_unatt_left(i) = sessionData(idx).control.unattendedRate_left;
        
        opto_hit_right(i) = sessionData(idx).opto.hitRate_right;
        opto_miss_right(i) = sessionData(idx).opto.missRate_right;
        opto_unatt_right(i) = sessionData(idx).opto.unattendedRate_right;
        opto_hit_left(i) = sessionData(idx).opto.hitRate_left;
        opto_miss_left(i) = sessionData(idx).opto.missRate_left;
        opto_unatt_left(i) = sessionData(idx).opto.unattendedRate_left;
    end
...
function plotpreStimLicking(sessionData, animalID)
    % Extract opto sessions only
    optoIdx = [];
    for i = 1:length(sessionData)
        if isfield(sessionData(i), 'isBaseline') && ~isempty(sessionData(i).isBaseline) && ~sessionData(i).isBaseline
    ...
    for i = 1:nSessions
        idx = optoIdx(i);
        ctrl_FA(i) = sessionData(idx).control.FARate * 100;
        opto_FA(i) = sessionData(idx).opto.FARate * 100;
    end
    ...
    ylabel('PreStim Licking Rate (%)', 'FontSize', 14);
    xlabel('Session', 'FontSize', 14);
    title(sprintf('%s - PreStim Licking Rates Across Sessions', animalID), ...
        'FontSize', 15, 'FontWeight', 'bold');
